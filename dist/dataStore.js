...
function updateUser(user) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        num_failed_passwords_since_last_login = $10 WHERE id = $11 RETURNING id`, [user.nameFirst, user.nameLast, user.email, user.password, user.streetName,
            user.cityName, user.postalZone, user.cbcCode, user.numSuccessfulLogins,
...
function addToken(token, userId) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield db_1.default.query("INSERT INTO Token (token, user_id) VALUES ($1, $2) RETURNING user", [token, userId]);
        return (res.rows.length > 0);
    });
}
...
function validToken(token) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield db_1.default.query("SELECT user FROM Token WHERE token = $1", [token]);
        return (res.rows.length > 0);
    });
}
...
function deleteToken(token) {
    return __awaiter(this, void 0, void 0, function* () {
        const res = yield db_1.default.query("DELETE FROM Token WHERE token = $1 RETURNING user_id", [token]);
        return (res.rows.length > 0);
    });
}
////////////////////////////////////////////////////////////////////////////////
...
function addOrder(order) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const billingRes = yield client.query("INSERT INTO BillingDetails (credit_card_no, cvv, expiry_date) VALUES ($1, $2, $3) RETURNING id", [billingDetails.creditCardNumber, billingDetails.CVV, billingDetails.expiryDate]);
            ...
            for (const index in order.items) {
                if (order.items[index].id === null) {
                    order.items[index].id = yield addItem(order.items[index]);
                }
                yield client.query("INSERT INTO OrderItems (order_id, item_id, quantity) VALUES ($1, $2, $3)", [orderId, order.items[index].id, order.quantities[index]]);
            }
...
function getOrder(orderId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const itemRes = yield db_1.default.query(`SELECT i.id, i.name, i.seller, i.description, i.price, oi.quantity FROM OrderItems oi
         JOIN Items i ON oi.item_id = i.id WHERE oi.order_id = $1`, [orderId]);
        const itemResults = [];
        const quantityResults = [];
        for (const item of itemRes.rows) {
            const sellerResult = yield getUserSimple(item.seller);
            if (sellerResult === null)
                return null;
            const itemResult = {
                id: item.id,
        ...
        const buyerResult = yield getUserSimple(order.user);
        if (buyerResult === null)
            return null;
...
function getOrdersByUser(userId) {
    return __awaiter(this, void 0, void 0, function* () {
        const orderRes = yield db_1.default.query("SELECT id FROM Orders WHERE buyer_id = $1", [userId]);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const billingRes = yield client.query(`UPDATE BillingDetails 
             ...
             WHERE id = (SELECT billing_id FROM Orders WHERE id = $4)`, [billingDetails.creditCardNumber, billingDetails.CVV, billingDetails.expiryDate, orderId]);
            const delivery = order.delivery;
            yield client.query(`UPDATE DeliveryInstructions 
            ...
            yield client.query(`UPDATE Orders 
             SET buyer_id = $1, last_edited = $2, status = $3, total_price = $4 WHERE id = $5`, [order.buyer.id, order.lastEdited, order.status, order.totalPrice, orderId]);
...
function deleteOrder(orderId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield client.query("BEGIN");
            const orderRes = yield client.query("DELETE FROM Orders WHERE id = $1", [orderId]);
            yield client.query("DELETE FROM OrderItems WHERE order_id = $1", [orderId]);
            const res = yield client.query("SELECT billing_id, delivery_id FROM Orders WHERE id = $1", [orderId]);
            if (res.rows.length === 0) {
                yield client.query("ROLLBACK");
                return false;
            }
...
function addOrderXML(orderId, xmlContent) {
    return __awaiter(this, void 0, void 0, function* () {
        const client = yield db_1.default.connect();
        try {
            yield client.query("INSERT INTO OrderXMLs (order_id, xml_content) VALUES ($1, $2)", [orderId, xmlContent]);
...
function getLatestOrderXML(orderId) {
    return __awaiter(this, void 0, void 0, function* () {
        const client = yield db_1.default.connect();
        try {
            const res = yield client.query(`SELECT xml_content FROM OrderXMLs WHERE order_id = $1
...
function getAllOrderXMLs(orderId) {
    return __awaiter(this, void 0, void 0, function* () {
        const client = yield db_1.default.connect();
        try {
            const res = yield client.query(`SELECT xml_content FROM OrderXMLs WHERE order_id = $1 ORDER BY created_at DESC`, [orderId]);
...
function clearItems() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield db_1.default.query("DELETE FROM Items");
            return true;
        }
        catch (err) {
...
function clearOrders() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield client.query("BEGIN");
            yield db_1.default.query("DELETE FROM Orders");
            yield db_1.default.query("DELETE FROM OrderItems");
            yield db_1.default.query("DELETE FROM BillingDetails");
            yield db_1.default.query("DELETE FROM DeliveryInstructions");
            yield db_1.default.query("DELETE FROM OrderXMLs");
            yield client.query("COMMIT");
            return true;
        }
        catch (err) {
