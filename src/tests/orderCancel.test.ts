...
describe("orderCancel successful return", () => {
  test.only("Should cancel an order successfully", async () => {
    ...
    // const UBL = await getLatestOrderXML(response.body.orderId);
    // console.log(UBL);
    const order = await getOrder(response.body.orderId); // <-- here!
    console.log(order);

    // const res = await getPutResponse(`/v1/${userId + 1000}/order/${orderId}/cancel`, {
